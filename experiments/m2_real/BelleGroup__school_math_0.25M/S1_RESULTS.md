...
## Round 2(v2 配对设计,进行中;run 31923540812)

| 组 | s42 | s43 | 均值 | vs 基座 48.3 |
|---|---|---|---|---|
| 基座 | 48.3(29/60) | — | — | — |
| raw | 46.7(28/60) | 待跑 | — | −1.6 |
| refined | 40.0(24/60) | 待跑 | — | **−8.3** |

**seed=42 单点观察(候选池实证:raw 1000 条,refined 946 条,留存 94.6%,与 S0
全量一致)**:配对设计修正混杂因素后,raw−refined 差距是 **6.7 分(4 题)**——
比 Round 1(有确认缺陷的设计)测出的 3.3 分**更大**,不是更小。这是有意义的
反面证据:如果 Round 1 的方向单纯是抽样混杂造成的假象,修正后差距应当缩小甚至
消失,而不是变大。目前判断:仍是单点,**不构成结论**,但"这是纯粹的抽样伪影"
这个解释的可信度在下降。seed=43(配对设计)已触发,若方向与幅度大致复现,
将按 M1-C1 先例扩到四种子(42/43/44/45)才可对外引用。
