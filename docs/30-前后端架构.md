...
│   ├── login/
│   │   ├── page.tsx           #   登录页(✅ 已落地,30-A)
│   │   └── actions.ts         #   Server Action:口令只经浏览器→Next 服务端→后端,零客户端 JS 落地
│   └── console/page.tsx       #   总览页(✅ 已落地,30-B 起点:余额+最近运行+订单,读专用)
├── components/                # 展示层(✅ 已落地):无状态 UI 原件,props 进类型出
│   ├── Card.tsx / StatChip.tsx
├── lib/
│   ├── api.ts                 # 客户端数据层:类型化 REST 客户端(Client Component 用,走 /df 反代)
│   ├── api-server.ts          # 服务端数据层(新增):Server Component 用,cookie→Bearer 转发,直连后端
│   ├── types.ts                # 契约层(✅ 已落地):镜像后端 Pydantic/store 响应形状
...
| 端点 | 方法 | 前端用途 | 响应形状(`lib/types.ts`) |
|---|---|---|---|
| `/auth/login` | POST | 登录页表单 | `{ token, role }` |
| `/auth/me` | GET | 会话自检 | `User` |
| `/auth/register` | POST | 注册页表单 | `{ username, role, next }` |
| `/auth/web` | GET | 免密直登落地(302 由后端处理,前端仅跳转) | — |
| `/billing/me` | GET | 账务页余额+流水 | `{ balance, orders: Order[] }` |
| `/billing/plans` | GET | 定价页 | `Record<string, Plan>` |
| `/billing/orders` | POST | 建单 | `{ order: Order, payment: PaymentIntent }` |
| `/datasets` | GET | 数据集列表 | `Dataset[]` |
| `/runs` | GET | 总览最近运行 | `RunSummary[]` |
| `/runs/{id}` | GET | Run 详情页 | `Run`(含 `manifest`) |
| `/runs/{id}/rejects` | GET | 死因分布/样例 | `{ rejects: RejectSample[] }` |
| `/runs/{id}/output` | GET | 下载交付(直接跳转,非 JSON) | — |
| `/recipes/{name}/history` | GET | 配方档案页 | `RecipeRun[]` |

前端**不消费**、也不应出现在任何前端代码里的端点:`/users`(管理面)、
`/billing/grant`(管理面)、`/billing/webhook`(服务器间回调)——三者continue 只走
...
- **环境变量契约**:前端仅需 `DF_API_BASE`(后端地址);后端对前端零感知
  (不存在"前端专属 env"这种耦合)
- **本地联调**:`uvicorn` 起后端 + `next dev` 起前端,`DF_API_BASE=http://localhost:8000`
  即可反代到本地后端;无需 mock,契约就是真实后端
...
| 前端无自动化测试 | 仅 `next build` 验证可编译 | 组件数超过一屏 → 引入 Playwright/Vitest,复用「看板走查」思路做 E2E |
