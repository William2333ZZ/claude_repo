...
│   └── console/
│       ├── page.tsx           #   总览页(✅,30-B:余额+最近运行+订单,读专用)
│       └── runs/[id]/page.tsx #   Run 详情页(✅,30-B:死因分布+人话+抽检+下载,浏览器实测)
...
| `/datasets` | GET | 数据集列表 | `Dataset[]` | 待接(30-B 数据集页) |
...
| `/recipes/{name}/history` | GET | 配方档案页 | `RecipeRun[]` | 待接(30-B) |
