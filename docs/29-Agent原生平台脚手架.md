...
写动作白名单外绝不长路径(写入口与 REST 共用同一函数);看板宕机,平台照常干活。
默认 SSR 零构建链,手机第一屏。〔锚〕docs/28 · service/console.py

### 3.6 默认选型与升级触发(反浪费:先证明需要,再引入)

| 域 | 默认 | 升级触发 |
|---|---|---|
| Web | FastAPI + SSR(零 node 构建链) | 交互复杂度超 SSR 舒适区 → SPA 另立项 |
...
| M4 | 看板:SSR 投影 + 免密直登 + 分享 | 手机走查:Landing → 分享 |
