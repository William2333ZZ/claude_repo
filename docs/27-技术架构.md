...
| 层 | 职责 | 技术 | 关键约束 |
|---|---|---|---|
| **T6 分发** | 把产品送进用户的 harness | SKILL.md 四件套(主)· argparse CLI · 自研 MCP stdio(~150 行,可选)· 控制台三页(f-string 渲染) | 技能纯文本零安装;控制台永不长出写路径 |
...
| 控制台认证 | **HTTP Basic** | 浏览器原生、零会话设施、直连 RBAC(HTTPS 前提) | 若需会话/SSO 再立项,现无 |
...
| 控制台 Basic 认证 | HTTPS 前提下可接受(只读) | 需会话/SSO 时再立项 |
